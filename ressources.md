---
layout: default
title: Ressources
---

# Quelques ressources à consulter

Pour apprendre à programmer en Python, il ne faut pas se limiter à ce cours.

Les ressources sont très nombreuses, quelques exemples :

## Des livres 
* l'excellent _Python pour les kids_ réservé aux débutants

![livre Python for Kids](../0_bases_1/images/python_for_kids.jpg#center)

## Des vidéos 
* de très nombreuses ressources sur _youtube_ comme _[la chaine de Jason Champagne](https://www.youtube.com/watch?v=HWxBtxPBCAc&list=PLrSOXFDHBtfHg8fWBd7sKPxEmahwyVBkC){:target="\_blank"}_
* [les pythonneries](https://www.dailymotion.com/playlist/x22t3u)  


## De nombreux sites comme :
* [python_doctor](https://python.doctor/)  
(sous forme de résumé des instructions, très pratique !!! Partie également sur Django, un framework pour fabriquer des sites Internet qui utilise Python , histoire de passer à autre chose quand on maîtrisera le serpent) 
* le site de _[Fabrice Sincère](http://fsincere.free.fr/isn/python/cours_python.php){:target="\_blank"}_ 
* le cours d'_[OpenClassRooms](https://openclassrooms.com/fr/courses/235344-apprenez-a-programmer-en-python){:target="\_blank"}_ (inscription gratuite)

## Les sites officiels (en anglais !)
* [Python](https://www.python.org/)
* [Pygame Zero](https://pygame-zero.readthedocs.io/en/stable/)
* [Pygame Zero](https://pygame-zero.readthedocs.io/en/stable/builtins.html)
* [Pygame](https://www.pygame.org/)


		



